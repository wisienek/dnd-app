...
import type { ContextCompatible, LoggerPort, LogMethod, OptionalParamObjectType } from '@dnd-app/core';
import { isArray, isObject } from 'lodash';
import { NodeEnv } from '@dnd-app/enums';
...
export abstract class Logger implements LoggerService, LoggerPort {
  ...
  public setContext(context: ContextCompatible | undefined) {
    if (context) {
      ...
      this.loggerContext = _context.slice(0, 1).toUpperCase() + _context.slice(1);
    }
  }
  ...
  error: LogMethod = (...args) => {
    ...
    this._logger.error(message, {
      ...metadata,
      context,
      error: this.isDev() ? error : { ...error, stack: error?.stack?.toString() },
    });
    if (error && this.isDev()) {
  ...
  private parseArgs(args: unknown[]) {
    ...
    if (!data.context && messages.length > 1 && messages.at(-1)?.match(/^[A-Z][a-z]*([A-Z][a-z]*)*$/)) {
