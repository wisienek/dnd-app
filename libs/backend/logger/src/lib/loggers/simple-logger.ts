...
import type { ContextCompatible } from '@dnd-app/core';
...
export class SimpleLogger extends Logger {
    ...
    return createLogger({
      transports: [
        new transports.Console({
          ...
          format: format.combine(format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss:SSSS' }), this.getLogFormat()),
        }),
      ],
    });
  }
  ...
  protected getLogFormat() {
    return this.isDev() ? new LocalLogFormat().get() : new ProductionLogFormat().get();
  }
  ...
  protected override getMetadata(withData = true): RequestMetadata {
    const metadata = {
      ...
      app:
        process.env['NX_TASK_TARGET_PROJECT']?.slice(process.env['NX_TASK_TARGET_PROJECT']?.indexOf('-') + 1) ||
